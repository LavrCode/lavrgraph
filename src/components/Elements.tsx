  ...
  switch (type) {
    case 'block-quote':
      return (
        <blockquote 
          {...attributes} 
          className={`border-l-4 border-indigo-400 dark:border-indigo-600 pl-4 italic my-5 dark:text-gray-300 dark:bg-gray-800 py-2 px-2 rounded ${alignClass}`}
        >
          {children}
        </blockquote>
      );
    case 'heading-one':
      return (
        <h1 {...attributes} className={`text-3xl font-bold my-6 dark:text-white ${alignClass}`}>
          {children}
        </h1>
      );
    case 'heading-two':
      return (
        <h2 {...attributes} className={`text-2xl font-bold my-5 dark:text-gray-100 ${alignClass}`}>
          {children}
        </h2>
      );
    case 'numbered-list':
      return (
        <ol {...attributes} className={`list-decimal pl-10 my-5 space-y-1 ${alignClass}`}>
          {children}
        </ol>
      );
    case 'bulleted-list':
      return (
        <ul {...attributes} className={`list-disc pl-10 my-5 space-y-1 ${alignClass}`}>
          {children}
        </ul>
      );
    case 'list-item':
      return <li {...attributes} className={`pl-1 ${alignClass}`}>{children}</li>;
    case 'image':
    ...
    default:
      return <p {...attributes} className={`my-3 dark:text-gray-200 leading-relaxed ${alignClass}`}>{children}</p>;
  }
};
