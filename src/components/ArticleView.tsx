  ...
  const renderContent = (content: Descendant[]) => {
    return content.map((node: any, index) => {
      if (node.type) {
        switch (node.type) {
          case 'paragraph':
            const paragraphClass = `my-3 dark:text-gray-200 leading-relaxed ${node.align ? `text-${node.align}` : ''}`;
            return <p key={index} className={paragraphClass}>{renderChildren(node.children)}</p>;
          case 'heading-one':
            const h1Class = `text-3xl font-bold my-6 dark:text-white ${node.align ? `text-${node.align}` : ''}`;
            return <h1 key={index} className={h1Class}>{renderChildren(node.children)}</h1>;
          case 'heading-two':
            const h2Class = `text-2xl font-bold my-5 dark:text-gray-100 ${node.align ? `text-${node.align}` : ''}`;
            return <h2 key={index} className={h2Class}>{renderChildren(node.children)}</h2>;
          case 'block-quote':
            const quoteClass = `border-l-4 border-indigo-400 dark:border-indigo-600 pl-4 italic my-5 dark:text-gray-300 dark:bg-gray-800 py-2 px-2 rounded ${node.align ? `text-${node.align}` : ''}`;
            return (
          ...
          case 'iframe':
            return (
              <div key={index} className="my-6 flex flex-col items-center">
                <div className="rounded-lg overflow-hidden shadow-md aspect-video max-w-2xl mx-auto w-full">
                  <iframe
                    src={node.src}
                    ...
                    loading="lazy"
                    title={node.caption || "Встроенный контент"}
                  ></iframe>
                </div>
                {node.caption && (
                  <div className="flex justify-center mt-2">
        ...
        {article && (
          <article className="prose prose-lg dark:prose-invert mx-auto">
            <div className="flex justify-between items-start mb-4">
              ...
              <Link 
                to={`/edit/${slug}`} 
                className="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-md ml-4 no-underline"
              >
                Редактировать
              </Link>
            </div>
