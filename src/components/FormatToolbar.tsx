...
const FormatToolbar: React.FC<FormatToolbarProps> = ({ className = '' }) => {
  ...
  const toggleBlock = (format: string) => {
    ...
    if (format === 'block-quote' && editor.selection && !Range.isCollapsed(editor.selection)) {
      ...
      if (selectedNodes.length > 1) {
        const texts = selectedNodes.map(([node]) => 
          Editor.string(editor, ReactEditor.findPath(editor, node as any))
        ).join('\n');
  ...
  const toggleAlign = (align: 'left' | 'center' | 'right') => {
    Transforms.setNodes(
      editor,
      { align },
      { match: n => !Editor.isEditor(n) && SlateElement.isElement(n) && !(n as any).type.includes('list') }
    );
  };
  ...
  const AlignButton = ({ align, icon, title }: { align: 'left' | 'center' | 'right', icon: string, title: string }) => {
