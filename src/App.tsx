...
const insertImage = (editor: CustomEditorWithImages, url: string, caption: string = '') => {
  ...
  if (editor.selection) {
    const [parentNode, parentPath] = SlateEditor.parent(editor, editor.selection.anchor.path);
    
    if (
      SlateElement.isElement(parentNode) &&
      parentNode.type === 'paragraph' &&
      SlateEditor.string(editor, parentPath) === ''
    ) {
...
const generateSlug = (title: string, articles: Article[]): string => {
  ...
  const similarArticles = articles.filter(article => {
    const articleBase = article.slug.split('-').slice(0, -4).join('-');
    return articleBase === slugBase;
  });
  ...
  return `${slugBase}-${dateStr}-${similarArticles.length + 1}`;
};
...
const withCorrectBlockHandling = (editor: CustomEditorWithImages) => {
  return editor;
};
...
const Editor: React.FC = () => {
  ...
  useEffect(() => {
    ...
    document.addEventListener('update-image-caption', handleCaptionUpdate as EventListener);
    
    return () => {
      document.removeEventListener('update-iframe-caption', handleCaptionUpdate as EventListener);
      document.removeEventListener('update-image-caption', handleCaptionUpdate as EventListener);
    };
  }, [editor]);
  ...
  const publishArticle = () => {
    ...
    .then(response => {
      if (!response.ok) {
        console.warn('Failed to save article to server, but saved locally', response.statusText);
      } else {
        console.log('Article saved to server and locally');
      }
    })
    .catch(error => {
      console.error('Error saving article to server:', error);
      console.warn('Article saved locally only');
    });
      ...
      <header className="dark:bg-gray-800 shadow-md py-4 px-4 sm:px-6 lg:px-8 sticky top-0 z-10">
        <div className="container mx-auto flex justify-between items-center">
          <div className="flex items-center">
            <h1 className="text-2xl font-bold dark:text-indigo-400">LavrGraph</h1>
          </div>
        </div>
      </header>
            ...
            <div className="max-w-4xl mx-auto p-4">
              ...
              >
                <div className="relative">
                  {/* Улучшенный фиксированный тулбар */}
  ...
  return (
    <Router>
      <Routes>
        ...
        <Route path="/edit/:slug" element={<EditArticleWrapper />} />
      </Routes>
    </Router>
  );
};

const EditArticleWrapper: React.FC = () => {
  const editor = useMemo(() => 
    withCorrectBlockHandling(
      withLinks(
        withImages(
